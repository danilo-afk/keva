...
- **Unpublished replies are posted by the harness.** Upstream drops any turn that ends in plain text without a `buzz messages send` (small models do this constantly, and DMs made it worse). `buzz-acp` now posts the agent's final text as the reply when nothing was published during the turn; set `BUZZ_ACP_PUBLISH_FINAL_TEXT=0` to get the upstream behaviour back.
...
- **Landing site (keva).** `kiara-site/` is the public site for ai-kiara.com: Next.js static export (`pnpm --filter kiara-site build` → `kiara-site/out/`), no login, no i18n, no backend. Routes: `/`, `/download`, `/pricing`, `/gallery`, `/terms`, `/privacy`. CTA target, contact e-mail and per-OS download links live in `kiara-site/src/lib/site.ts`; a `null` download link renders "Request early access". Deployed to S3 + CloudFront, DNS on Cloudflare. It is not the relay `web/` client and must never reference Buzz or `buzz://`.
- **Productions (keva).** A production is a kind `30180` event (`docs/nips/NIP-KP.md`): slug, name, format, aspect, status, the bible (`context`), its channels (`c` tags) and its agents (`agent` tags with optional per-production instructions). Create one from the sidebar *Productions → +* (wizard creates `<slug>-general/story/visual` channels and deploys the chosen team into each), or seed the examples from *Productions → ⋮ → Create example productions*. The Overview edits the bible, adds channels, applies a team and sets per-agent instructions. `buzz-acp` injects the production as a `<production>` section into every new agent session in a production channel; edits are picked up on the next new session. CLI: `buzz productions list|get|create`. Long material is split into entities (`docs`, `episodes` with shot lists and per-shot state, `characters` with reference images) — `buzz productions docs|episodes|characters list|get|set|delete --slug <slug>`; the production page shows them as Documents / Episodes (collapsible blocks, search and state filter, start time of every episode in the sequence, Table or Board view) / Characters / Assets / Animatic (uploaded cuts with versions, the current one playable, plus the sequence timeline), and the harness gives agents the document index so they fetch what they need. The production page has Edit/Delete (⋯ menu; delete is a tombstone, channels and entities stay), and every image opens in a lightbox with remove/add. Built-in team **Kiara Studio** (Producer, Argumento, Bíblia personas) knows this CLI contract: apply it to a production from the Team tab. `scripts/import-jony/` is not shipped; the importer used for the Jony pre-production HTML lives with the project.
