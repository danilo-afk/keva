...
fn dev_keyring_service(configured: Option<String>) -> String {
    configured
        .filter(|service| service.starts_with("ai-kiara.com (dev)."))
        .unwrap_or_else(|| "ai-kiara.com (dev)".to_string())
}
...
pub(super) fn migration_marker_name(service: &str, default_name: &str) -> String {
    if service == "ai-kiara.com" || service == "ai-kiara.com (dev)" {
...
mod tests {
    ...
    fn standalone_scope_must_remain_under_dev_service() {
        assert_eq!(
            dev_keyring_service(Some("ai-kiara.com (dev).example".to_string())),
            "ai-kiara.com (dev).example"
        );
        assert_eq!(
            dev_keyring_service(Some("ai-kiara.com".to_string())),
            "ai-kiara.com (dev)"
        );
    }
    ...
    fn standalone_scope_uses_its_own_migration_marker() {
        assert_eq!(
            migration_marker_name("ai-kiara.com", "identity.migrated"),
            "identity.migrated"
        );
        assert_eq!(
            migration_marker_name("ai-kiara.com (dev)", "identity.migrated"),
            "identity.migrated"
        );
        assert_eq!(
            migration_marker_name("ai-kiara.com (dev).example", "identity.migrated"),
            "identity.ai-kiara.com (dev).example.migrated"
        );
    }
}
