...
test("parseSupportedLinkPreview parses Kiara relay git clone URLs", () => {
...
test("parseSupportedLinkPreview rejects malformed Kiara git URLs", () => {
  for (const href of [
...
test("parseSupportedLinkPreview rejects clone URLs from non-relay hosts", () => {
  ...
  // github.com sharing the path shape must never become a Kiara repo card.
  assert.equal(
    parseSupportedLinkPreview(
...
test("extractSupportedLinkPreviews excludes Kiara entity links while keeping external links", () => {
  const entityLinks = [
    `buzz://project?owner=${BUZZ_OWNER}&d=buzz-world`,
...
test("extractSupportedLinkPreviews excludes markdown-labeled Kiara entity links", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
...
test("extractSupportedLinkPreviews excludes same-relay Kiara clone URLs", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
...
test("extractSupportedLinkPreviews excludes markdown-labeled Kiara clone URLs", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
      `[Kiara World](https://relay.example/git/${BUZZ_OWNER}/buzz-world-galaxy)`,
      "https://relay.example",
    ),
    [],
  );
